...
/*** Header description: ***
 * In bytes: (4)+ 3+ 1+ 1+ 4= 2+ 9 Bytes.  (*4 - for every 1 byte in () as every 4 bytes of of master file contain 2 bits)
...
public class Steganograph {
	...
	public boolean embedMessage(File masterFile, File outputFile, String msg, int compression, String password) {
		...
		try
		{
			...
			if (byteArrayIn.length < inputOutputMarker)
			{
				JOptionPane
						.showMessageDialog(null, "Message size is bigger then input wav file. Please Select a bigger wav file.", "Audio Steganography", JOptionPane.ERROR_MESSAGE);
				return false;
			}
		...
		{
			JOptionPane.showMessageDialog(null, "Message size is bigger then input wav file. Please Select a bigger wav file.", "Audio Steganography", JOptionPane.ERROR_MESSAGE);
			return false;
		}
